    ...
    <div class="container">
        ...
        <?php if ($arResult['COUNT_PRODUCTS'] > 0) :
            ...
            foreach ($arResult['ITEMS'] as $key => $items) : ?>
                <div class="basket-store-block" id="<?= ($num === 0) ? 'bid-license__open' : '' ?>">
                    ...
                    if (is_array($items)) {
                        foreach ($items as $item) : ?>
                            ...
                            >
                                <div class="product-checkbox">
                                    <input class="product-checkbox__input" type="checkbox">
                                </div>
                                <?php if ($item['PICTURE']) : ?>
                                    <div class="product-image-block"><a href="<?= $item['LINK'] ?>">
                                        <img src="<?= $item['PICTURE'] ?>" alt="<?= $item['NAME'] ?>">
                                    </a>
                                        <div class="product-delivery-text">
                                            <?php foreach ($item['STORE_ID'] as $stores) { ?>
                                                <?php if (!empty($stores)) { ?>
                                                    <span><?=$stores['STORE_TITLE']?></span>
                                                <?php } ?>
                                            <?php } ?>
                                        </div>
                                    </div>
                        ...
                        <div class="bottom-store-block">
                            ...
                            <div class="store-total-button-block">
                                <button <?= ($countItems == $countCantBuy) ? 'disabled' : '' ?> class="btn btn-lg btn-primary <?= ($num !== 0) ? 'basket-btn-checkout' : ''?>" <?= ($num === 0) ? "id=bid-license-btn__open" : '' ?>>

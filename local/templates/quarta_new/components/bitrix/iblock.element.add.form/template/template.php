...
<form name="iblock_add" action="<?= POST_FORM_ACTION_URI ?>" method="post" enctype="multipart/form-data">
	...
	<? endif; ?>
	<? if ($arResult["MESSAGE"] <> ''): ?>
		<div class="bid-license__close"><?= buildSVG('close', SITE_TEMPLATE_PATH . '/assets/icons/') ?></div>
		<? if ($arResult["MESSAGE"] === 'Элемент успешно добавлен'): ?>
			<p><?= Loc::getMessage('IBLOCK_FORM_FORM_SUCCESS');?></p>
		<? else: ?>
			<? ShowNote($arResult["MESSAGE"])?>
		<? endif; ?>
	<? else: ?>
		<div class="bid-license__close"><?= buildSVG('close', SITE_TEMPLATE_PATH . '/assets/icons/') ?></div>
		<h2 class="bid-license__title"><?= Loc::getMessage('IBLOCK_FORM_FORM_TITLE'); ?></h2>
		<?= bitrix_sessid_post() ?>
		<? if ($arParams["MAX_FILE_SIZE"] > 0): ?>
			<input type="hidden" name="MAX_FILE_SIZE" value="<?= $arParams["MAX_FILE_SIZE"] ?>" />
		<? endif ?>
		<div class="bid-license__data">
			<? if (is_array($arResult["PROPERTY_LIST"]) && !empty($arResult["PROPERTY_LIST"])): ?>

				<? foreach ($arResult["PROPERTY_LIST"] as $propertyID): ?>
					<div class="input input--full-name mb-4 <? if (in_array($propertyID, $arResult['PROPERTY_REQUIRED'])): ?>input--required <? endif ?>">
						<? if ($arResult["PROPERTY_LIST_FULL"][$propertyID]["NAME"] !== 'Товар' && $propertyID !== 'NAME'): ?>
							...
							switch ($INPUT_TYPE):
								...
								case "N":
									for ($i = 0; $i < $inputNum; $i++) {
										...
										<? if ($arResult["PROPERTY_LIST_FULL"][$propertyID]["NAME"] == 'Email'): ?>
											<input type="email" class="form-control" name="PROPERTY[<?= $propertyID ?>][<?= $i ?>]" size="<?= $arResult["PROPERTY_LIST_FULL"][$propertyID]["COL_COUNT"]; ?>" value="<?= $value ?>" />
										<? elseif ($arResult["PROPERTY_LIST_FULL"][$propertyID]["NAME"] == 'Товар'): ?>
											<input type="hidden" class="form-control" name="PROPERTY[<?= $propertyID ?>][<?= $i ?>]" size="<?= $arResult["PROPERTY_LIST_FULL"][$propertyID]["COL_COUNT"]; ?>" value="<?= $value ?>" />
										<? elseif ($propertyID == "NAME"): ?>
											<input type="hidden" class="form-control" name="PROPERTY[<?= $propertyID ?>][<?= $i ?>]" size="<?= $arResult["PROPERTY_LIST_FULL"][$propertyID]["COL_COUNT"]; ?>" value="<?= 'Заявка ' . date("j-n-Y H:i:s") ?>" />
										<? else: ?>
								...
								case "F":
									...
									?>
										...
										?>
											<input type="checkbox" name="DELETE_FILE[<?= $propertyID ?>][<?= $arResult["ELEMENT_PROPERTIES"][$propertyID][$i]["VALUE_ID"] ? $arResult["ELEMENT_PROPERTIES"][$propertyID][$i]["VALUE_ID"] : $i ?>]" id="file_delete_<?= $propertyID ?>_<?= $i ?>" value="Y" /><label for="file_delete_<?= $propertyID ?>_<?= $i ?>">
											...
											?>
												...
												[<a href="<?= $arResult["ELEMENT_FILES"][$value]["SRC"] ?>">
													<?= GetMessage("IBLOCK_FORM_FILE_DOWNLOAD") ?>
												</a>]
											<?
											}
										}
								...
								case "L":
									...
									switch ($type):
										...
										case "radio":
											...
											?>
												<input type="<?= $type ?>" name="PROPERTY[<?= $propertyID ?>]<?= $type == "checkbox" ? "[" . $key . "]" : "" ?>" value="<?= $key ?>" id="property_<?= $key ?>" <?= $checked ? " checked=\"checked\"" : "" ?> /><label for="property_<?= $key ?>">
													<?= $arEnum["VALUE"] ?>
												</label>
											<?
											}
				...
				<? if ($arParams["USE_CAPTCHA"] == "Y" && $arParams["ID"] <= 0): ?>
					<div class="bid-license__captcha">
			...
			<div class="privacy-block">
				<input required="" type="checkbox" id="privacy-text" name="privacy-text" required>
			...
			<div class="bid-license__btn">
				<input type="submit" name="iblock_submit" class="btn btn-primary w-100" value="<?= GetMessage("IBLOCK_FORM_SUBMIT") ?>" />
				<? if ($arParams["LIST_URL"] <> ''): ?>
					<input type="submit" name="iblock_apply" class="btn btn-primary w-100" value="<?= GetMessage("IBLOCK_FORM_APPLY") ?>" />
					<input
